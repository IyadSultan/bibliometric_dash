...
# Add database connection classes
...
class BibliometricData:
    """Handles data retrieval from SQL views."""
    def __init__(self):
        self.db = DatabaseConnection()

    def get_papers_summary(self):
        """Get papers summary from vw_bibliometric_papers_summary."""
        with self.db.get_connection() as conn:
            query = """
                SELECT 
                    ...
                    concepts,
                    mesh_terms,
                    CAST('[]' AS NVARCHAR(MAX)) as topics  -- Add default empty topics array
                FROM vw_bibliometric_papers_summary
                ORDER BY publication_date
            """
            df = pd.read_sql(query, conn)
            
            # Rename journal to journal_name to match existing code
            df = df.rename(columns={'journal': 'journal_name'})
            
            # Add citations_per_year calculation (avoid dividing by zero)
            df['publication_year'] = df['publication_year'].fillna(0).astype(int)
            df['citations_per_year'] = df.apply(
                lambda row: row['citations'] / (2024 - row['publication_year'])
                if row['publication_year'] != 0 else 0,
                axis=1
            )
            
            # Ensure topics is a string containing empty JSON array if null
...
def load_data():
    ...
    try:
        ...
        print(f"Loaded {len(authors_df)} KHCC author records.")
...
# Basic data preprocessing for papers_df
...
# Basic data preprocessing for authors_df
...
if not papers_df.empty:
    ...
    print("No data available to create figures")
    fig_pubs = fig_cites = fig_quartile_trend = None
...
def create_topics_wordcloud(papers_df):
    """Create wordcloud from topics with empty fallback."""
    try:
        # Initialize empty word frequency dictionary
        word_freq = {}
        
        # Process each paper's topics
        for _, paper in papers_df.iterrows():
                    ...
                    if isinstance(topics, list):
                        for topic in topics:
                            if isinstance(topic, dict) and 'display_name' in topic:
                                ...
                                word_freq[topic_name] = word_freq.get(topic_name, 0) + 1
                except json.JSONDecodeError:
                    continue
        
        # If no topics found, add a default message
        if not word_freq:
            word_freq = {"No topics available": 1}
        ...
        # Return a minimal blank image in case of error
